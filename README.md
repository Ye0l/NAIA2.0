...
**메모리** — NAIA는 검색 풀 전체를 메모리에 올립니다. 수백만 행짜리 실제 태그 풀이면
풀·스냅샷·필터 인덱스가 모두 살아 있을 때 수 GB가 됩니다. `docker-compose.yml`의
`mem_limit`을 **이미지가 아니라 다루는 풀 크기에 맞춰** 잡으세요.

> **⚠️ 풀이 컨테이너 메모리보다 크면 앱이 아예 못 뜹니다.**
> 시작 시 마지막 풀을 디스크에서 복원하는데, 이때 한도를 넘으면 커널이 프로세스를
> SIGKILL 합니다 — 파이썬이 손쓸 수 없고, `restart: unless-stopped`가 그걸 재시작
> 루프로 만듭니다. `.env`에 `NAIA_SKIP_POOL_RESTORE=1` 을 넣으면 빈 풀로 기동하니,
> 올라온 뒤 더 작은 풀을 불러오거나 검색을 새로 돌리면 됩니다.

`NAIA_POOL_LOAD_ROWS_IN_FLIGHT`는 풀 로딩이 디코드 중에 들고 있을 행 수입니다.
올리면 행그룹을 더 여러 개 동시에 풀어 빨리 읽고, 스레드 하나당 행그룹 하나만큼
메모리를 더 씁니다.
